    ...
    return (
      ...
      >
        <Block
          ...
          onTouchStart={actions.setActive(true)}
          borderRadius={9999}
          overflow="hidden"
          bgColor="#CCC"
          h={barHeight}
        ...
        {active && (
          <Window
            onMouseUp={[decodeRaw(actions.drag), actions.setActive(false)]}
            onTouchEnd={[decodeRaw(actions.drag), actions.setActive(false)]}
            onMouseMove={decodeRaw(actions.drag)}
            onTouchMove={decodeRaw(actions.drag)}
          />
        )}
      </Block>
    );
